  ...
  Query: {
    ...
    users: async (_, {}, { user }) => {
      ...
      try {
        ...
        let userdata = await admindetails.findOne({
          uuid: user,
        });
        return userdata;
      } catch (error) {
    ...
    tenentpayment: async (_, args, {}) => {
      const { uuid } = args;
      try {
        const result = await paymentDetails.findOne({
          $or: [{ tenantuuid: uuid }, { roomuuid: uuid }],
        });
        console.log(result);
        return result;
      } catch (error) {
    ...
    totalrooms: async (_, {}, { user }) => {
      try {
        const data = await roomDetails.findOne({ adminuuid: user });
        return data?.roomDetail;
      } catch (error) {
        throw new Error(error.message);
      }
    },
    roomDetails: async (_, { roomuuid }, { user }) => {
      try {
        // const data = await roomDetails?.findOne({
        //   adminuuid: adminuuid,
        // });
        // const room = data.roomDetail.find((r) => r.name === "rooms");
        // const result = room.detail.find((r) => r.uuid === roomuuid);
        const result = await tenantDetails.findOne({ roomuuid: roomuuid });
        return result;
      } catch (error) {
        throw new Error(error.message);
      }
    ...
    getIssues: async (_, args, { user }) => {
      const uuid = user;
      try {
        const exist = await problems.find({ adminuuid: uuid }).toArray();
        if (!exist) {
          throw new Error("User did not found");
        }
  ...
  Mutation: {
    ...
    onLogin: async (_, { input }) => {
      ...
      try {
        let exist;
        if (email === "admin@49") {
          exist = await admindetails.findOne({ useremail: email });
        } else {
          exist = await tenantDetails?.findOne({
    ...
    Riseissues: async (_, { input }) => {
      const { tenantuuid, roomuuid, question } = input;
    ...
    addtentant: async (_, { input }, { user }) => {
      ...
      try {
        ...
        if (result) {
          ...
          if (addrentpage) {
            const adminuuid = user;
            const addtenanttoroom = await roomDetails?.findOneAndUpdate(
              { adminuuid: adminuuid, "roomDetail.name": `${type}s` },
              {
                $set: {
                  "roomDetail.$.detail.$[room].tenantuuid": uuid,
                },
              },
              {
                arrayFilters: [{ "room.name": doornumber }],
                new: true,
              }
            );
            if (addtenanttoroom) {
